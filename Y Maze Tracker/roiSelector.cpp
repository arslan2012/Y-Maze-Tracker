...
namespace {
    class ROISelector {
    public:
        Rect select(const LPCWSTR& windowName, Mat img, bool showCrossair = true, bool fromCenter = true) {
            // show notice to user
            ...
            // select the object
            cvSetMouseCallback(windowName, mouseHandler, (void*)this);
            ...
            while (!(key == 32 || key == 27 || key == 13)) {
                ...
                cvShowImage(windowName, selectorParams.image);
            ...
            //cleanup callback
            cvSetMouseCallback(windowName, emptyMouseHandler, NULL);

            return selectorParams.box;
        }

        void select(const LPCWSTR& windowName, Mat img, std::vector<Rect>& boundingBoxes,
...
Rect selectROI(InputArray img, bool showCrosshair, bool fromCenter) {
    ...
    return selector.select(L"ROI selector", img.getMat(), showCrosshair, fromCenter);
}

Rect selectROI(const LPCWSTR& windowName, InputArray img, bool showCrosshair, bool fromCenter) {
    ROISelector selector;
    return selector.select(windowName, img.getMat(), showCrosshair, fromCenter);
}

void selectROIs(const LPCWSTR& windowName, InputArray img,
